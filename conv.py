...
class Conv2d(_ConvNd):
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor]):
        if self.padding_mode != 'zeros':
            return F.conv2d(F.pad(input.to(torch.float16), self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            weight.to(torch.float16), bias.to(torch.float16), self.stride,
                            _pair(0), self.dilation, self.groups)
        return F.conv2d(input.to(torch.float16), weight.to(torch.float16), bias.to(torch.float16), self.stride,
                        self.padding, self.dilation, self.groups)
