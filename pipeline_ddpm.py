...
class DDPMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if isinstance(self.unet.config.image_size, int):
            image_shape = (
                batch_size,
                self.unet.config.in_channels,
                self.unet.config.image_size,
                self.unet.config.image_size,
            )
        else:
            image_shape = (batch_size, self.unet.config.in_channels, *self.unet.config.image_size)
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            # print('time.device', time.device)
            # print('mask.device', mask.device)

            model_output = self.unet(image, time, mask).sample
